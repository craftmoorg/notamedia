...
class Custom_CIBlockCMLImport extends CIBlockCMLImport
{
    private $oldElementSectionPropId = '';
    ...
    function ImportElement($arXMLElement, &$counter, $bWF, $arParent)
    {
        ...
        elseif(isset($arXMLElement[$this->mess["IBLOCK_XML2_NAME"]]))
        {
            ...
            $arElement = $this->SaveOldSectionsForElement($arElement, $this->next_step["IBLOCK_ID"]);
    ...
    function SaveOldSectionsForElement($arElement1c, $iblockId = '')
    {

        if (!empty($iblockId)) {
            $arElement = \CIBlockElement::GetList([], ['XML_ID' => $arElement1c['XML_ID'], 'IBLOCK_ID' => $iblockId, 'ACTIVE' => 'Y'], false, false, ['ID', 'PROPERTY_OLD_IBLOCK_SECTION_ID'])->Fetch();

            $elementId = $arElement['ID'];
            $elementOldIblockSectionId = $arElement['PROPERTY_OLD_IBLOCK_SECTION_ID_VALUE'];
            if (empty($this->oldElementSectionPropId)){
                $this->oldElementSectionPropId = CIBlockProperty::GetList([], ['ACTIVE' => 'Y', 'IBLOCK_ID' => $iblockId, 'CODE' => 'OLD_IBLOCK_SECTION_ID'])->Fetch()['ID'];
            }

            $oldGroups = CIBlockElement::GetElementGroups($elementId, true);
            while ($arGroup = $oldGroups->Fetch()) {
                $oldSections[] = $arGroup['ID'];
            }

            // Формируем массив разделов от 1с и старых разделов
            $newSections = array_unique(array_merge($arElement1c['IBLOCK_SECTION'], $oldSections));

            // Проверяем старую привязку товара к разделу из 1с
            if (empty($elementOldIblockSectionId)){
                foreach ($arElement1c['PROPERTY_VALUES'][$this->oldElementSectionPropId] as &$array){
                    if (array_key_exists('VALUE', $array)){
                        $array['VALUE'] = $arElement1c['IBLOCK_SECTION_ID'];
                    }
                }
                unset($array);
            }
            elseif ($arElement1c['IBLOCK_SECTION_ID'] != $elementOldIblockSectionId){
                foreach ($arElement1c['PROPERTY_VALUES'][$this->oldElementSectionPropId] as &$array){
                    if (array_key_exists('VALUE', $array)){
                        $array['VALUE'] = $arElement1c['IBLOCK_SECTION_ID'];
                    }
                ...
                unset($array);
                unset($newSections[array_search($elementOldIblockSectionId, $newSections)]);
            }
            $arElement1c['IBLOCK_SECTION'] = $newSections;
			$arElement1c['IBLOCK_SECTION'][] = $arElement1c['IBLOCK_SECTION_ID'];
			$arElement1c['IBLOCK_SECTION'] = array_unique($arElement1c['IBLOCK_SECTION']);
